...
@Component({
  selector: 'app-doctor-reclamations',
  templateUrl: './doctor-reclamations.component.html',
  styleUrls: ['./doctor-reclamations.component.css']
})
export class DoctorReclamationsComponent implements OnInit {
  doctorId = 2;
  ...
  loadReclamations(): void {
    this.reclamationService.getByTargetRole('DOCTOR').subscribe({
      next: (data) => {
      ...
      error: () => {
        this.errorMessage = 'Erreur lors du chargement des réclamations du médecin.';
      }
    });
  }
  ...
  updateStatus(id?: number, status?: any): void {
    ...
    this.reclamationService.updateStatus(id, status, this.doctorId, 'DOCTOR').subscribe({
      next: () => {
