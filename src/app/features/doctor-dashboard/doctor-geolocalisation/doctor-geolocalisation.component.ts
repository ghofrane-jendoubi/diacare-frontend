...
@Component({
  selector: 'app-doctor-geolocalisation',
  templateUrl: './doctor-geolocalisation.component.html',
  styleUrls: ['./doctor-geolocalisation.component.css']
})
export class DoctorGeolocalisationComponent implements OnInit, AfterViewInit {
  private map!: L.Map;
  ...
  selectedFilter: 'all' | 'pharmacie' | 'laboratoire' = 'all';
  searchTerm = '';
  favoritesOnly = false;
  ...
  private initMap(): void {
    this.map = L.map('doctor-map').setView([36.8065, 10.1815], 13);
    ...
    this.geoService.getNearbyPlaces(this.userLat, this.userLng, this.radiusMeters).subscribe({
      next: (places: Place[]) => {
        this.places = places.filter(
          (place: Place) =>
            place.type === 'pharmacie' || place.type === 'laboratoire'
        );
        this.applyFilters();
        this.loading = false;
      },
  ...
  toggleFavorite(place: Place): void {
    ...
    localStorage.setItem('doctor_geo_favorites', JSON.stringify(this.favoriteIds));
    this.applyFilters();
  }
  ...
  private loadFavorites(): string[] {
    const raw = localStorage.getItem('doctor_geo_favorites');
