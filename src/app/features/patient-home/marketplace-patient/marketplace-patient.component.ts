...
closeDetailsModal(): void {
  ...
  analyzeProduct(): void {
    ...
    if (this.analyzerMode === 'barcode') {
      ...
      this.http.get(`http://localhost:8080/api/ml/analyze-barcode/${this.barcode}`)
        .subscribe({
          next: (result: any) => {
      ...
      this.http.post('http://localhost:8080/api/ml/analyze-name', { productName: this.productName })
        .subscribe({
          next: (result: any) => {
            this.processAnalysisResult(result);
            this.isAnalyzing = false;
      ...
      this.http.post('http://localhost:8080/api/ml/analyze-image', formData)
        .subscribe({
          next: (result: any) => {
            this.processAnalysisResult(result);
            this.isAnalyzing = false;
