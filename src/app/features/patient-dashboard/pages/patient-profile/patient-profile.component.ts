...
loadPatientInfo(): void {
  ...
  loadProfileFromApi(): void {
    ...
    this.nutritionService.getNutritionProfile(this.patientId)
      .subscribe({
        ...
        error: () => console.log('⚠️ Aucun profil nutrition trouvé')
      });
  }
  ...
  saveProfile(): void {
    ...
    const data = {
      ...
      bmi:           this.imc,
      targetCalories: this.dailyCalories,
      targetCarbs:   this.dailyCarbs,
      targetProtein: Math.round((this.dailyCalories! * 0.25) / 4),
      targetFat:     Math.round((this.dailyCalories! * 0.3) / 9)
    };
  ...
  deleteProfile(): void {
    ...
    if (confirm('Êtes-vous sûr de vouloir supprimer votre profil nutritionnel ?')) {
