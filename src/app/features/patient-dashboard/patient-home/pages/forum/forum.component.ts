...
export class ForumComponent implements OnInit {
  ...
  this.forumService.createPost({
    ...
    patientName: patientName  // ← Utiliser le nom correctement formaté
  }).subscribe({
    next: (res) => {
    ...
    error: (err) => {
      ...
      this.postError = err.error?.message || 'Erreur de connexion';
    }
  });
}
  ...
  console.log('Envoi commentaire:', commentData); // Debug

  this.forumService.addComment(this.selectedPost.id, commentData).subscribe({
    next: (res) => {
      ...
      if (res.blocked) {
        ...
        this.comments.push(res.comment || res);
    ...
    error: (err) => {
      ...
      if (err.error) {
        ...
        alert(`Erreur: ${err.error.message || 'Données invalides'}`);
      } else {
